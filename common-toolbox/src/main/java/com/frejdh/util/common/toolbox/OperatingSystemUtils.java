...
public class OperatingSystemUtils {
	 ...
	 *
	 * @param file The full file path. Only the last filename will have replaced characters.
	 * @return A new string with the illegal characters replaced
	 */
	public static File replaceIllegalFilenameCharacters(File file) {
		File parentDirectory = file != null ? file.getParentFile() : null;
		if (file == null || parentDirectory == null) {
			return null;
		} else if (StringUtils.isBlank(parentDirectory.getPath())) {
			return new File(replaceIllegalFilenameCharacters(file.getPath()));
		}

		return new File(parentDirectory, replaceIllegalFilenameCharacters(file.getPath()));
	}
