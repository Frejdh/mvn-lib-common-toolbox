...
public class Conditionals<T, R> {
	...
	/**
	 * The original value. Set early during {@link #execute()}
	 */
	 ...
	 */
	@SafeVarargs
	public final EqualsValueCondition<T, R> equalsToAny(@Nullable T... equalsToValue) {
	 ...
	 */
	public EqualsValueCondition<T, R> equalsTo(@Nullable T equalsToValue) {
	 ...
	 * Keep in mind, that {@link Predicate} statements can be stacked in the event that it shall support multiple predicates in one check.
	 */
	public PredicateCondition<T, R> fulfills(@Nonnull Predicate<T> predicate) {
	...
	public final PredicateCondition<T, R> equalsToPredicate(@Nonnull Predicate<T> equalsToPredicate) {
	...
	public R execute() {
		...
		Optional<AbstractCondition<T, R>> matchingCondition = equalsAndPredicateConditions.stream()
				.filter(condition -> isFulfillingEqualsCondition(condition, finalValue))
				.findFirst();
		return (matchingCondition.isPresent()) ? matchingCondition.get().returnValue.get() : attemptCast(null);
	}

	/**
	 * Executes the operation based on the configured conditions.
	 * If the resolved value is {@code null}, then it returns the original value.
	 */
	public T executeOrReturnSelf() {
		R retval = execute();
		if (retval != null) {
			return attemptCastToSourceValue(retval);
		}
	...
	protected R attemptCast(T value) {
		try {
			...
			throw new IllegalStateException("Cannot cast to the return type as the parameter value retrieved and the returned element type are mismatching", e);
		}
	}

	@SuppressWarnings("unchecked")
	protected T attemptCastToSourceValue(R value) {
		try {
			return (T) value;
		} catch (ClassCastException e) {
			throw new IllegalStateException("Cannot cast to the return type as the parameter value retrieved and the returned element type are mismatching", e);
